...
class MeshInterfaceApp:
    ...
    def __init__(self, root, mesh_function, on_mesh_created=None):
        ...
        """
        self.root = root
        self.root.title("Interface para create_3d_mesh")
    ...
    def create_widgets(self):
        ...
        """
        # ==================================================================
        # Usar grid para o layout principal da janela
        # ==================================================================
        self.root.grid_rowconfigure(0, weight=1)  # Linha principal expande
        self.root.grid_rowconfigure(1, weight=0)  # Linha dos botões não expande
        self.root.grid_columnconfigure(0, weight=1)  # Toda a coluna expande
        
        # Container principal (contém tudo exceto os botões)
        main_container = ttk.Frame(self.root)
        main_container.grid(row=0, column=0, sticky="nsew")
        
        # Painel para botões - sempre fixo na parte inferior
        ...
        # Configuração de grid para os frames de controle
        left_frame.columnconfigure(0, weight=1)
        left_frame.rowconfigure(0, weight=0)  # Parâmetros da malha não expande
        left_frame.rowconfigure(1, weight=0)  # Adicionar intervalos não expande
        left_frame.rowconfigure(2, weight=1)  # Intervalos ativos expande
        ...
        params_frame.grid(row=0, column=0, sticky="new", padx=5, pady=5)
        
        # Layout em grade vertical para os parâmetros
        params_frame.columnconfigure(0, weight=1)
        params_frame.columnconfigure(1, weight=1)
        
        ttk.Label(params_frame, text="nx:").grid(row=0, column=0, padx=5, pady=2, sticky=tk.W)
        ttk.Spinbox(params_frame, from_=1, to=40, textvariable=self.nx_var, width=8).grid(row=0, column=1, padx=5, pady=2, sticky=tk.W)
        
        ttk.Label(params_frame, text="ny:").grid(row=1, column=0, padx=5, pady=2, sticky=tk.W)
        ttk.Spinbox(params_frame, from_=1, to=40, textvariable=self.ny_var, width=8).grid(row=1, column=1, padx=5, pady=2, sticky=tk.W)
        
        ttk.Label(params_frame, text="nz:").grid(row=2, column=0, padx=5, pady=2, sticky=tk.W)
        ttk.Spinbox(params_frame, from_=1, to=40, textvariable=self.nz_var, width=8).grid(row=2, column=1, padx=5, pady=2, sticky=tk.W)
        ...
        interval_input_frame.grid(row=1, column=0, sticky="new", padx=5, pady=5)
        
        # Layout em grade vertical para os intervalos
        ...
        for row, (label, var) in enumerate(entries):
            ttk.Label(interval_input_frame, text=label).grid(
                row=row, column=0, padx=5, pady=2, sticky=tk.W
            )
            spin = ttk.Spinbox(interval_input_frame, textvariable=var, width=8)
            spin.grid(row=row, column=1, padx=5, pady=2, sticky=tk.W)
            
            # Guardar referências para atualizar limites mais tarde
            if row == 0:
                self.z_spinbox = spin
            elif row == 1:
                self.x_spinbox = spin
        ...
        ).grid(row=len(entries), column=0, columnspan=2, padx=5, pady=5, sticky=tk.EW)
        ...
        interval_display_frame.grid(row=2, column=0, sticky="nsew", padx=5, pady=5)
        
        # Configuração para que a treeview expanda
    ...
    def update_spinbox_limits(self, *args):
        ...
        *args
        ...
        try:
            ...
            self.z_spinbox.config(from_=0, to=max(0, nz-1))
            self.x_spinbox.config(from_=0, to=max(0, nx-1))
            self.y_start_spinbox.config(from_=0, to=max(0, ny-1))
            self.y_end_spinbox.config(from_=0, to=max(0, ny-1))
            
            # Ajusta os valores atuais para estarem dentro dos novos limites
            if self.z_layer_var.get() >= nz:
                self.z_layer_var.set(max(0, nz-1))
            if self.x_line_var.get() >= nx:
                self.x_line_var.set(max(0, nx-1))
            if self.y_start_var.get() >= ny:
                self.y_start_var.set(max(0, ny-1))
            if self.y_end_var.get() >= ny:
                self.y_end_var.set(max(0, ny-1))
...
def open_mesh_interface(mesh_function=None):
    """
    Abre a interface gráfica e retorna a malha gerada.
    
    Esta função é o ponto de entrada principal para usar a interface gráfica.
    Inicializa a interface, processa eventos e retorna a malha gerada.
    
    Parameters
    ----------
    mesh_function : callable, optional
        Função que cria a malha 3D. Se None, usa a função create_3d_mesh do módulo.
    
    Returns
    -------
    numpy.ndarray or None
        A malha 3D gerada pela interface ou None se nenhuma malha foi gerada
    """
    # Se não for fornecida uma função, use a que está definida no módulo
    if mesh_function is None:
        mesh_function = create_3d_mesh
    
    # Variável para armazenar a malha
    mesh_result = [None]
    
    # Função para capturar a malha quando criada
    def capture_mesh(mesh):
        mesh_result[0] = mesh
    
    # Configurar e iniciar a interface
    root = tk.Tk()
    app = MeshInterfaceApp(root, mesh_function, on_mesh_created=capture_mesh)
    root.mainloop()
    
    # Retornar a malha capturada
