...
class MainApp:
    ...
    def create_refined_mesh_interface(self):
        ...
        #self.refined_mesh_app = RefinementInterfaceApp(root=self.refined_mesh_tab, mesh=np.zeros((5,5,5)), 
        #refine_function=refine_mesh, on_refinement_complete=self.store_refined_mesh)
        self.refined_mesh_app = RefinementInterfaceApp(root=self.refined_mesh_tab, mesh=np.zeros((1,1,1)), refine_function=refine_mesh, on_refinement_complete=self.store_refined_mesh)
    ...
    def store_mesh(self, mesh):
        ...
        self.refined_mesh_app.update_mesh_info_display()
        ...
        self.refined_mesh_app.update_mesh_info_display() # Ensure info is updated after visualization        
...
#        self.refined_mesh_app.update_spinbox_limits() # Probably wrong, then commented
       ...
       # Update spinbox limits in RefinementInterfaceApp
#        self.refined_mesh_app.update_refinement_spinbox_limits()
